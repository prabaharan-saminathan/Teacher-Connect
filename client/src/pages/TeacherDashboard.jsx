  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const appointmentsResponse = await customFetch.get(
  ...
  const handleStatusUpdate = async (appointmentId, status) => {
    try {
      const { data } = await customFetch.patch(
        `/appointments/teacher/appointments/${appointmentId}`,
        {
          status,
        }
      );
              ...
              <div className="text-center sm:text-left flex-1">
                {isEditing ? (
                  <form
                    ...
                    className="bg-white/10 backdrop-blur-md p-6 rounded-xl border border-white/20 shadow-xl space-y-6"
                  >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              ...
              <span className="badge badge-primary text-gray-300">
            ...
            <div className="space-y-4 max-h-[calc(100vh-300px)] overflow-y-auto pr-2 custom-scrollbar">
              {appointments.map((appointment) => (
                <div
                  key={appointment._id}
                  className={`bg-gray-500 rounded-xl shadow-sm hover:shadow-md transition-all duration-200 cursor-pointer ${
